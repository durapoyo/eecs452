...
AudioEffectGranular granularAL; 
AudioEffectGranular granularAR;
AudioEffectGranular granularBL; 
AudioEffectGranular granularBR;
...
void handleTempoControls() {
 ...
 if (rollRate > TEMPO_THRESH) { playbackRate += 0.05f; if(playbackRate>2.0f) playbackRate=2.0f; granularAL.setSpeed(playbackRate); granularAR.setSpeed(playbackRate); Serial.printf("Tempo UP → %.2fx\n", playbackRate); confirmGesture(); }
 else if (rollRate < -TEMPO_THRESH) { playbackRate -= 0.05f; if(playbackRate<0.5f) playbackRate=0.5f; granularAL.setSpeed(playbackRate); granularAR.setSpeed(playbackRate); Serial.printf("Tempo DOWN → %.2fx\n", playbackRate); confirmGesture(); }
}
 ...
 switch (gesture) {
  ...
  break;
   ...
   if (!eqMode){
	...
	tempoMode = false;
	mixerMode = false;
	Serial.println("EQ mode activated");
   }
   else{
	eqMode = false;
	Serial.println("EQ mode deactivated. Back to Normal mode");
   }
   case 4: 
  nextSongA();
  break;
  ...
  mixerMode = !mixerMode;
  if (mixerMode) {
	Serial.println("Mixer Mode ON (controls locked)");
	if (!isPlayingB) {
	...
	Serial.println("Mixer Mode OFF (controls unlocked)");
  ...
  break;
   case 6: 
	...
	tempoMode = false;
	Serial.println("Tempo mode deactivated. Back to normal mode");
   }
   case 7: 
  nextSongB();
  break;
